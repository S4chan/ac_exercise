...
function renderFriendList(data) {
  let rawHTML = ''
  if (!data.length) {
    rawHTML = '<p class="text-center">沒有符合的結果</p>'
  } else {
    data.forEach(item => {
...
function renderPaginator(amount) {
  ...
  for (let page = 1; page <= numberOfPages; page++) {
    rawHTML += `<li class="page-item"><a class="page-link text-light" href="#" data-page="${page}">${page}</a></li>`
  }
...
searchInput.addEventListener('input', function onSearchInputChanged(event) {
  ...
  if (!filteredFriends.length) {
    dataPanel.innerHTML = '<p class="text-center">沒有符合的結果</p>'
    paginator.innerHTML = '' // 清空分頁器
  } else {
